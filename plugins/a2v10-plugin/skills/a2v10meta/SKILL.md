...
- **Write `$schema` in every `metadata.json`.** The pointer is relative to the application — `"../../@schemas/metadata-endpoint-json-schema.json#"`, or the file of its own kind (`accplan`, `ledger`, `enum`, `state`, `report`, `autonum`; `menu.json` and `app.json` have theirs). **That folder is not yours to write and is not kept in this skill:** the `A2v10.App.Assets2026` package lays it into the project, so it is there after the first build and moves with the package on update. What it buys is what the bullet above says the loader will not give you: a key written into the wrong place, or spelled in the wrong casing, is marked in the editor before anything is deployed. It checks the shape of the file, not whether the declaration is right — the rules are here and in `references/`.
