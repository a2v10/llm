...
- **`dataModel.filters`** — per root array, the filters the procedure echoed in `$System` via `!Filter` (`sql-rules.md`): filter name → `{ type }`, where `type` follows the marker form — `value` (scalar), `period` (the `Period.From`/`Period.To` pair as one node), `reference` (+ `refType`: the `T…` it points to). Absent when the procedure echoes none — an `edit` has no filters by nature. These are the names the view's `FilterItem Property="…"` and `{Bind …Filter.<name>}` must use; a name not here is not a filter, whatever the view binds.
- **`$meta` in a slot** — `model`, a `sqlProcedures` verb, `view.file`, `template.file` — means the slot is built by another component and has no file behind it (SKILL.md §3, *`$meta` → not yours*). An element `model.json` does not declare may still resolve this way. For such an element `dataModel` is the shape handed to you — the only source for your binds, and the stale caveat above does not apply to it.
