...
A2v10 fails **silently** at runtime: a name that doesn't bind raises no error — the runtime simply
...
exception: they fail **loudly**, and the user brings you the text. Most are
project-file problems — compare against
`../scaffold/MainApp/MainApp.csproj` in this skill, which is already correct.
...
| No syntax highlighting / completion in `.vxaml` (Visual Studio) | Install the A2v10 Xaml Language Service extension (Visual Studio 2026+) if missing: https://marketplace.visualstudio.com/items?itemName=OleksandrKukhtin.a2v10xamlsp. Completion reads the `@assemblies` folder, filled by the BeforeBuild target of `A2v10.App.Assets2026`: no folder → build the project; still none → compare the package reference with the scaffold csproj. `.xaml` files are not covered — never rename them to `.vxaml`. |

**Never resolve a build error by putting the application project back on
...
- **Template** — `template:` names an existing `.template.ts`, and its `.js` was rebuilt after the last edit (SKILL.md §4)?
