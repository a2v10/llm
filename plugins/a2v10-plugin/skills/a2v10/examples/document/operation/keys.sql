...
go
------------------------------------------------
if not exists(select * from INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS where CONSTRAINT_NAME = 'FK_Documents_Agent_Agents')
	alter table doc.Documents add
...
------------------------------------------------
if not exists(select * from INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS where CONSTRAINT_NAME = 'FK_DocDetails_Document_Documents')
...
go
------------------------------------------------
if not exists(select * from INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS where CONSTRAINT_NAME = 'FK_DocDetails_Item_Items')
	alter table doc.DocDetails add
...
-- FK від doc.DocLinks
------------------------------------------------
if not exists(select * from INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS where CONSTRAINT_NAME = 'FK_DocLinks_ParentId_Documents')
	alter table doc.DocLinks add
		constraint FK_DocLinks_ParentId_Documents foreign key (ParentId) references doc.Documents(Id);
go
------------------------------------------------
if not exists(select * from INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS where CONSTRAINT_NAME = 'FK_DocLinks_ChildId_Documents')
	alter table doc.DocLinks add
		constraint FK_DocLinks_ChildId_Documents foreign key (ChildId) references doc.Documents(Id);
go
------------------------------------------------
if not exists(select * from INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS where CONSTRAINT_NAME = 'FK_DocLinks_LinkId_OpLinks')
	alter table doc.DocLinks add
